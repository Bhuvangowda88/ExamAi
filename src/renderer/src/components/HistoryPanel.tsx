  ...
  return (
    <div className="flex-1 flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-medium text-slate-200">History</h2>
        <button
          onClick={onClose}
      ...
      {items.length === 0 ? (
        ...
        <div className="flex-1 overflow-y-auto space-y-3 pr-1">
          {items.map((item) => (
            <div key={item.id} className="bg-slate-900/40 border border-slate-700/40 rounded-lg p-3">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs text-slate-500">{formatTimestamp(item.createdAt)}</span>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => onUse(item)}
                    className="flex items-center gap-1 text-xs px-2 py-1 rounded bg-slate-800/70 hover:bg-slate-700 text-slate-200"
      ...
      {items.length > 0 && (
        <button
          onClick={onClear}
          className="mt-4 w-full py-2 bg-slate-900/60 hover:bg-slate-800 text-slate-300 text-xs font-medium rounded-lg transition-colors"
        >
          Clear History
        </button>
      )}
